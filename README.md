# Тестовое задание стажеру службы поддержки пользователей. Решение.
...
### Заявка о проблеме почтового клиента, если это Outlook.
1. **Причина: Outlook не подключен к Интернету.**
...
2. **Причина: для некоторых учетных записей электронной почты POP и IMAP, использующих SMTP, требуется проверка подлинности.**
...
4. **Сети необходим прокси-сервер для подключения к почтовому серверу.**
...
5. **Письма не приходят из-за фильтра защиты от нежелательной почты.**
...
6. **Наличие подозрительных писем в учетной записи Outlook.**
...
7. **Outlook конфликтует с антивирусным ПО.**
...
8. Надстройки не позволяют Outlook получать электронные письма.
...
### Решение проблемы с доставкой писем на Linux с неизвестным почтовым клиентом
...
```
### Решения:
#### 1. Вывод данной команды укажет на синтаксические ошибки:
```bash
postfix check mail*.conf
```
#### 2. Возможно нужный порт блокируется 2.1.другим приложением. 2.2.firewall 2.3.антивирусом. Нам нужны следующие порты: 25, 110(для POP3), 465 и 587
...
#### 2.2 Отключите антивирус. Протестируйте.
#### 2.3. Для начала можно проверить настройки DNS-сервера. Если не поможет.
...
#### 3. Если определенные письма доходят, а соедение до прочих сайтов устанавливается успешно, то предлагаю опустить этот пункт.
